...
public class MosaicIntCol extends MosaicGenerator<IntColorVector, BufferedImage, BufferedImage, BufferedImage> {
  ...
  protected long compare(IntColorVector tile, IntColorVector image) {
    ...
    for (int i = 0; i < tile.a.length; i+=4) {
      double cf = ((double) tile.a[i] + image.a[i]) / (2*255);
      int da = Math.abs(tile.a[i] - image.a[i]) * 3;
      int db = Math.abs(tile.b[i] - image.b[i]);
      int dg = Math.abs(tile.g[i] - image.g[i]);
      int dr = Math.abs(tile.r[i] - image.r[i]);
      ret += (long) ((1.0 - cf) * da + cf * (db + dg + dr));
    }
    return ret;
  }
